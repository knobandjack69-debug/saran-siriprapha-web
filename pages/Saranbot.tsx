...
import { Send, Bot, User, ArrowRight, ShieldAlert, Sparkles, Trash2, CheckCircle2, Search, BrainCircuit } from 'lucide-react';
...
export const Saranbot: React.FC<SaranbotProps> = ({ content }) => {
  ...
  useEffect(() => {
    ...
    if (isLoading) {
      ...
      }, 400); // High-speed interval (400ms) for snappier feel
    }
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          temperature: 0.4, // Slightly lower temperature for more focused, faster responses
          thinkingConfig: { thinkingBudget: 0 } // Bypass thinking steps for maximum speed
        },
      });

      const botMessage: Message = {
        role: 'bot',
        text: response.text || 'ขออภัยครับ เกิดข้อผิดพลาดบางประการ',
        timestamp: new Date(),
      };
          ...
          {messages.map((m, i) => (
            ...
            >
              <div className={`w-12 h-12 rounded-2xl flex-shrink-0 flex items-center justify-center border shadow-lg transform hover:rotate-12 transition-transform ${
                ...
                : 'bg-white border-party-black/5 text-party-rose'
              }`}>
